...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, inputs):
        # convert inputs from BCHWL -> BHWLC #batch height width length channel
        ...
        return loss, quantized.permute(0, 4, 1, 2, 3).contiguous(), perplexity, encodings, encoding_indices
...
class Encoder(nn.Module):
    def __init__(self, in_channels, num_hiddens, num_residual_layers, num_residual_hiddens, num_downsample_layers, extraConv=False):
        ...
        self._bn_1 = nn.BatchNorm3d(num_hiddens // (2**(self.num_downsample_layers - 1)))

        self._downConvs = nn.ModuleList([nn.Conv3d(in_channels=num_hiddens // (2**(self.num_downsample_layers - 1 - m)),
                                                   out_channels=num_hiddens // (2**(self.num_downsample_layers - 2 -m)),
                                        kernel_size=4,stride=2, padding=1)
                                        for m in range(self.num_downsample_layers - 1)])
        
        self._bn_down = nn.ModuleList([nn.BatchNorm3d(num_hiddens // (2**(self.num_downsample_layers - 2 -m))) for m in range(self.num_downsample_layers - 1)])
        ...
        self._bn_3 = nn.BatchNorm3d(num_hiddens)

        if self.doExtraConv:
            if num_downsample_layers > 10:
                #self._extraConv = nn.Conv3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[3, 4, 4], stride=[1, 2, 2], padding=[1, 1, 1])
                #self._extraConv = nn.Conv3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[3, 4, 4], stride=[1, 1, 1], padding=[2, 0, 0]) #This is to go from 3x4x4 to 5x1x1
                self._extraConv = nn.Conv3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[1, 4, 4], stride=[1, 1, 1], padding=[0, 0, 0]) #This is to go from 3x4x4 to 3x1x1
                #self._extraConv = nn.Conv3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[3, 4, 4], stride=[1, 2, 2], padding=[0, 1, 1]) #This is to go from 6x8x8 to 4x4x4
            else:
                #self._extraConv = nn.Conv3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[4, 1, 1], stride=[2, 1, 1], padding=[1, 0, 0]) #This is to only halve the first dimension
                self._extraConv = nn.Conv3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[1, 4, 4],
                                            stride=[1, 2, 2],
                                            padding=[0, 1, 1])  # This is to only halve non- first dimension
            self._bn_extra = nn.BatchNorm3d(num_hiddens)
    ...
    def forward(self, x_):
        ...
        x_ = self._residual_stack(x_) #Should I put an activation before this?

        #print("eoncoder end shape", x_.shape)
        return x_
...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels, num_hiddens, num_residual_layers, num_residual_hiddens, num_downsample_layers, extraConv):
        ...
        self._bn_1 = nn.BatchNorm3d(num_hiddens)
        ...
        if self.doExtraConv:
            if num_downsample_layers > 10:
                #self._extraUpConv = nn.ConvTranspose3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[3, 4, 4], stride=[1, 2, 2], padding=[1, 1, 1])
                #self._extraUpConv = nn.ConvTranspose3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[3, 4, 4], stride=[1, 1, 1], padding=[2, 0, 0]) #This is to go from 5x1x1 from 3x4x4
                self._extraUpConv = nn.ConvTranspose3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[1, 4, 4], stride=[1, 1, 1], padding=[0, 0, 0]) #This is to go from 3x1x1 from 3x4x4
                #self._extraUpConv = nn.ConvTranspose3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[3, 4, 4], stride=[1, 2, 2], padding=[0, 1, 1]) #This is to go from 4x4x4 from 6x8x8
            else:
                #self._extraUpConv = nn.ConvTranspose3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[4, 1, 1], stride=[2, 1, 1], padding=[1, 0, 0]) #Halve the first dimension
                self._extraUpConv = nn.ConvTranspose3d(in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=[1, 4, 4],
                                            stride=[1, 2, 2],
                                            padding=[0, 1, 1])  # This is to only halve non- first dimension
            self._bn_extra = nn.BatchNorm3d(num_hiddens)
            

        self._upConvs = nn.ModuleList([nn.ConvTranspose3d(in_channels=num_hiddens // 2 ** (m),
                                                out_channels=num_hiddens // 2 ** (m + 1),
                                                kernel_size=4,
                                                stride=2, padding=1)
                                                for m in range(self.num_downsample_layers - 1)])
        
        self._bn_up = nn.ModuleList([nn.BatchNorm3d(num_hiddens // 2 ** (m + 1)) for m in range(self.num_downsample_layers - 1)])

        self._conv_trans_1 = nn.ConvTranspose3d(in_channels=num_hiddens // 2 ** (self.num_downsample_layers - 1),
    ...
    def forward(self, x_):
        x_ = self._conv_1(x_)
        x_ = self._bn_1(x_)
        x_ = self._residual_stack(x_) #Don't need an activation here right before residual stack
        if self.doExtraConv:
...
class VQVAE(nn.Module):
    def __init__(self, config):
        ...
        self._pre_vq_conv = nn.Conv3d(in_channels=config.num_hiddens,
    ...
    def decode_from_c(self, encoding_indices):
        ...
        # Quantize and unflatten
        quantized = torch.matmul(encodings, self._vq_vae._embedding.weight).view(self._vq_vae._input_shape).permute(0, 4, 1, 2, 3).contiguous()
        x_recon = self._decoder(quantized)
        return x_recon
...
class Discriminator(nn.Module):
    def __init__(self, in_features=1, last_conv_kernalsize=4, verbose=False):
